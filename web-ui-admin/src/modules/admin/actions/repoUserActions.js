...
export const getRepoUser = id => dispatch => {
  ...
  const url = `${serviceUrl}/api/repo/repoUser/${id}`;
  console.log(url)
  axios
    .get(url)
    .then(res => {
...
// Search repoUsers
export const getRepoUsers = (searchTerm, adminEmail) => dispatch => {
  dispatch(setRepoUserLoading());
  const serviceUrl = process.env.REACT_APP_REPO_SERVICE_URL;
  searchTerm = !searchTerm ? '*' : searchTerm;
  const url = `${serviceUrl}/api/repo/searchUsersByAdmin/${searchTerm}/${adminEmail}`;
